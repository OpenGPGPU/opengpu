...
class BTB(val parameter: BTBParameter)
  ...
  println(s"core inst bytes log2 ${log2Up(coreInstBytes)}")
  ...
  when(r_btb_update.valid) {
    ...
    printf(cf" update target ${update_target} \n")
    printf(cf" tgts warr data ${update_target(matchBits - 1, log2Up(coreInstBytes))} \n")
